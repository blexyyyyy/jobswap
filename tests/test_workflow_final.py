...
try:
    ...
    from database.db_manager import init_db, insert_job
...
# Step 5: Test vector store and database insertion
...
try:
    ...
    # Try to insert using the same format as fetch_and_seed
    try:
        job_id = insert_job(job_data, raw_text)
        if job_id:
            print(f"  ✓ Job inserted with ID: {job_id}")
        else:
            print(f"  ⚠ Job already exists in database (duplicate)")
    except Exception as insert_err:
        print(f"  ⚠ Job insert returned error: {insert_err}")
